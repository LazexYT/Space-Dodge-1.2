...
# Update the load_user_data function to load rocket settings
def load_user_data():
    global users, user_scores, user_rocket_settings
...
# Update the save_user_data function to save rocket settings
def save_user_data():
    with open(USER_DATA_FILE, 'w') as f:
        for username, password in users.items():
            ...
            # Get rocket settings or use defaults
            rocket_data = user_rocket_settings.get(username, {
                "style": 0,
                "primary_color": RED,
                "secondary_color": BLUE
            })
            
            rocket_style = rocket_data["style"]
            primary_color = rocket_data["primary_color"]
            secondary_color = rocket_data["secondary_color"]
            
            # Convert RGB tuples to strings
            primary_color_str = f"{primary_color[0]}:{primary_color[1]}:{primary_color[2]}"
            secondary_color_str = f"{secondary_color[0]}:{secondary_color[1]}:{secondary_color[2]}"
            
            f.write(f"{username},{password},{easy_score},{medium_score},{hard_score},{rocket_style},{primary_color_str},{secondary_color_str}\n")
...
def settings_menu():
    ...
    # Volume settings (default to 100%)
    volume_level = 100
    ...
    volume_handle_rect = pygame.Rect(700, 335, 20, 60)  # Position at max initially
...
def icons_menu():
    ...
    global current_rocket_style, primary_color, secondary_color, current_user
    
    # Store initial settings to check for changes later
    initial_style = current_rocket_style
    initial_primary = primary_color
    initial_secondary = secondary_color
    ...
    # Create settings option buttons
    icons_button = pygame.Rect(button_x, 300, button_width, button_height)
    primary_button = pygame.Rect(button_x, 400, button_width, button_height)
    secondary_button = pygame.Rect(button_x, 500, button_width, button_height)
    button_rect_back = pygame.Rect(50, 50, 150, 80)  # Back button
    ...
    style_buttons = []
    for i in range(len(rocket_styles)):
        style_buttons.append(pygame.Rect(
            WIDTH/2 - 350 + i * 250,
            300,
            200,
            60
    ...
    # Track which section is active
    active_section = "main"  # main, icons, primary, secondary
    ...
    while running:
        ...
        # Draw login status
        WIN.blit(login_status, (WIDTH - login_status.get_width() - 20, 20))
        
        # Draw rocket preview in all sections
        draw_rocket(WIN, WIDTH - 150, 600, 80, 160, current_rocket_style, primary_color, secondary_color)
        preview_text = small_font.render("Current Design:", True, WHITE)
        WIN.blit(preview_text, (760, 450))
        
        # Main settings screen
        if active_section == "main":
            # Title
            title_text = font.render("ROCKET CUSTOMIZER", True, WHITE)
            WIN.blit(title_text, (WIDTH/2 - title_text.get_width()/2, 150))
            
            # Draw option buttons
            pygame.draw.rect(WIN, BLACK, icons_button)
            pygame.draw.rect(WIN, BLACK, primary_button)
            pygame.draw.rect(WIN, BLACK, secondary_button)
            
            # Button texts
            icons_text = med_font.render("Choose Style", True, WHITE)
            primary_text = med_font.render("Primary Color", True, WHITE)
            secondary_text = med_font.render("Secondary Color", True, WHITE)
            
            WIN.blit(icons_text, (icons_button.centerx - icons_text.get_width()/2, 
                               icons_button.centery - icons_text.get_height()/2))
            WIN.blit(primary_text, (primary_button.centerx - primary_text.get_width()/2, 
                                 primary_button.centery - primary_text.get_height()/2))
            WIN.blit(secondary_text, (secondary_button.centerx - secondary_text.get_width()/2, 
                                   secondary_button.centery - secondary_text.get_height()/2))
            
        # Icons/Style settings screen
        elif active_section == "icons":
            # Title
            title_text = font.render("ROCKET STYLE", True, WHITE)
            WIN.blit(title_text, (WIDTH/2 - title_text.get_width()/2, 150))
            
            # Draw style options
            for i, button in enumerate(style_buttons):
                pygame.draw.rect(WIN, BLACK, button)
                style_text = med_font.render(rocket_styles[i], True, WHITE)
                WIN.blit(style_text, (button.centerx - style_text.get_width()/2, 
                                   button.centery - style_text.get_height()/2))
                
                # Draw small preview for each style
                draw_rocket(WIN, button.centerx - 20, button.bottom + 30, 40, 80, i, RED, BLUE)
            
            # Highlight the selected style
            pygame.draw.rect(WIN, GREEN, style_buttons[current_rocket_style], 3)
            
        # Primary color screen
        elif active_section == "primary":
            # Title
            title_text = font.render("PRIMARY COLOR", True, WHITE)
            WIN.blit(title_text, (WIDTH/2 - title_text.get_width()/2, 150))
        ...
        # Secondary color screen
        elif active_section == "secondary":
            # Title
            title_text = font.render("SECONDARY COLOR", True, WHITE)
            WIN.blit(title_text, (WIDTH/2 - title_text.get_width()/2, 150))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if current_user and (initial_style != current_rocket_style or 
                                    initial_primary != primary_color or 
                                    initial_secondary != secondary_color):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # Handle main menu button clicks
                if active_section == "main":
                    if icons_button.collidepoint(event.pos):
                        active_section = "icons"
                    elif primary_button.collidepoint(event.pos):
                        active_section = "primary"
                    elif secondary_button.collidepoint(event.pos):
                        ...
                        if button.collidepoint(event.pos):
                            ...
                            if current_user:
                                if current_user not in user_rocket_settings:
                                    ...
                                    user_rocket_settings[current_user]["style"] = current_rocket_style
                                save_user_data()
                ...
                # Handle secondary color selection
                    ...
                    for i, button in enumerate(color_buttons):
                        if button.collidepoint(event.pos):
                            secondary_color = color_options[i]
                            ...
                            if current_user:
                                if current_user not in user_rocket_settings:
                                    ...
                                    user_rocket_settings[current_user]["secondary_color"] = secondary_color
                                save_user_data()
